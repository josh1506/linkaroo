...
class URL(ActivatorModel, Model):
    ...
    def total_clicks(self):
        return self.tracker.all().count()

    @property
    def latest_total_clicks_7_days(self):
        seven_days_ago = timezone.now() - timezone.timedelta(days=7)
        return self.tracker.filter(created_at__gte=seven_days_ago)

    @property
    def is_active_url(self):
        return self.tracker.exists()
